...
// ========== PROFIT TOAST ==========
window.showProfitToast = function(msg) {
  const toast = document.getElementById('profitToast');
  const text = document.getElementById('profitToastText');
...
async function completeInvestment(investId) {
  ...
  try {
    showProfitToast('+$' + totalReturn.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + ' ' + inv.plan + ' investment matured!');
  } catch (e) {
    console.warn('Profit toast failed:', e);
  }

  // Animate balance update
  try {
    const balWrap = document.getElementById('balanceWrapper');
    if (balWrap) {
      balWrap.classList.add('balance-pop');
      setTimeout(() => balWrap.classList.remove('balance-pop'), 500);
    }
  } catch (e) {
    console.warn('Balance animation failed:', e);
  }
...
// ========== AUTO CLAIM CHECKER ==========
function startAutoClaimChecker() {
  if (typeof checkInvestmentLock !== 'function') {
    console.warn('checkInvestmentLock not available, skipping auto-claim');
    return;
  }
  try {
    checkInvestmentLock();
  } catch (e) {
    console.warn('Initial checkInvestmentLock failed:', e);
  }
  setInterval(() => {
    try {
      checkInvestmentLock();
    } catch (e) {
      console.warn('Auto-claim check failed:', e);
    }
  }, 30000);
}
...
window.submitTransfer = async function(event) {
  ...
  try {
    ...
    // Trigger transfer coin animation
    const coin = document.getElementById('transferCoin');
    if (coin) {
      coin.style.left = '50%';
      coin.style.top = '50%';
      coin.textContent = '$' + Math.floor(amount);
      coin.classList.add('animate');
      setTimeout(() => coin.classList.remove('animate'), 1100);
    }

    // Show transfer success toast
    showProfitToast('Transfer of $' + amount.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + ' sent to ' + recipientEmail);
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // GUARANTEED: hide loading overlay after max 4s no matter what
  const hideLoader = () => {
  ...
  try {
    ...
    // Start auto-claim checker for matured investments (non-blocking)
    try {
      startAutoClaimChecker();
    } catch (e) {
      console.warn('Auto-claim checker failed to start:', e);
    }
